...
def render_login():
    ...
    if enc_pass == account["password"]:
        session_var = {"user": user_id,
                       "role": account["role"], "username": account["username"]}
...
def submit_add_user():
    if 'user' in session:
        ...
        for row in temp_df.itertuples():
            ...
            dbo.create_user(username, fname, lname, role,
                            encrypt_sha256(username+password), email)
            userlist.append(username)
            subject = "NEW USER REGISTERED TYPE -{} ID: {} ".format(
                role, username)
...
def get_user_detail_by_userID_sheet():
    if 'user' in session:
        ...
        user_detail = dbo.get_user_detail_by_userID_sheet(
            basic_details['USERNAME']).to_dict('records')
...
def submit_cleaning_room_report():
    ...
    file_name = "Cleaning_room_report_{}.xlsx".format(
        str(datetime.datetime.today().strftime('%d_%m_%Y')))
...
def getProductData():
    if 'user' in session:
        ...
        product_frame = dbo.get_product_details_by_version(
            basic_details['version_number'])
        product_list = product_frame.to_dict('records')
