...
function submit_Report()
{
	...
	for(var i =0; i<code_rows.length; i++)
	{
			...
			for(var j = 1; j<header_length; j++)
			{
				...
				{
					data[i][j] =code_rows[i].children[j].children[0].firstElementChild.value
				}
				else
				{
	...
	on();
	$.getJSON('/submit_cleaning_room_report', 
	{
...
function view_report_log()
{
	...
	}, function(result) 
	{
		...
		{	var prepared_by   = report_list[j].prepared_by;
			...
			<td><input type="text" name="report_number" value='+report_number+' class="textfield" disabled></td>\
			<td><a href='+link+' download='+report_list[j].file_name+'>'+report_list[j].file_name+'</a><td></tr>'		
...
function edit_report(version_number)
{
	...
	$.getJSON('/getProductData', 
	{
		params_data : JSON.stringify(basic_details)
	}, function(result) 
	{
		report_list =  result['product_log']
		var header='<th>Product Name</th>\
		<th>Generic Name</th>\
		<th>Form</th>\
		...
		{				  			
			var temp = '<tr>\
			<td><input type="text" name="Product_Name" value='+report_list[j].Product_Name+' class="textfield" disabled></td>\
			<td><input type="text" name="Generic_Name" value='+report_list[j].Generic_Name+' class="textfield" disabled></td>\
			<td><input type="text" name="Form" value='+report_list[j].Form+' class="textfield" disabled></td>\
			<td><input type="text" name="API_with_strength" value='+report_list[j].API_with_strength+' class="textfield" disabled></td>\
			<td><input type="text" name="Minimum_Batch_size_NOS" value='+report_list[j].Minimum_Batch_size_NOS+' class="textfield" disabled></td>\
			<td><input type="text" name="Minimum_Batch_size_MG" value='+report_list[j].Minimum_Batch_size_MG+' class="textfield" disabled></td>\
			<td><input type="text" name="MRDD" value='+report_list[j].MRDD+' class="textfield" disabled></td>\
			<td><input type="text" name="LRDD_MG" value='+report_list[j].LRDD_MG+' class="textfield" disabled></td>\
			<td><input type="text" name="LRDD_NOS" value='+report_list[j].LRDD_NOS+' class="textfield" disabled></td>\
			<td><input type="text" name="PDE_VALUE" value='+report_list[j].PDE_VALUE+' class="textfield" disabled></td>\
			<td><input type="text" name="LD50" value='+report_list[j].LD50+' class="textfield" disabled></td>\
			<td><input type="text" name="NOEL" value='+report_list[j].NOEL+' class="textfield" disabled></td>\
			'
			$('#ReportTable').append(temp);	
			$('#DownlodReportbtn').show();
		}
...
function view_logbook()
{
	...
	}, function(result) 
	{
		...
		var header='<tr><th>VERSION</th><th>STATUS</th><th>UPDATED BY</th><th>VIEW DATA</tr>'
		
		$('#LOGTABLE').append(header);
		for(var j = 0; j<record_list.length; j++)
		{	
			var temp = '<tr id="myTableRow" name="myTableRow">\
			<td><input type="text" name="VERSION" value='+record_list[j].VERSION+' class="textfield" disabled></td>\
			<td><input type="text" name="STATUS" value='+record_list[j].STATUS+' class="textfield" disabled></td>\
			<td><input type="text" name="UPDATED_BY" value='+record_list[j].UPDATED_BY+' class="textfield" disabled></td>\
			<td><button style="background-color:#ffcc00" onclick="edit_report(\'' +record_list[j].VERSION+  '\')" >VIEW DATA</button>\
			</td>\
			</tr>'
